...
enum LearnStep { case modeSelect, settings, flashcard, conversation }
enum LearnPracticeMode { case flashcard, conversation }
...
struct LearnModeView: View {
    ...
    var body: some View {
        NavigationStack {
            Group {
                if cards.isEmpty { emptyStateView }
                else {
                    switch step {
                    case .modeSelect:    modeSelectView
                    case .settings:      settingsView
                    case .flashcard:     flashcardSessionView
                    case .conversation:  conversationSessionView
                    }
                }
            }
            ...
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button {
                        if step == .modeSelect { speechService.stop(); convVM.stop(); dismiss() }
                        else { withAnimation { goBack() } }
                    } label: {
                        if step == .modeSelect { Image(systemName: "xmark") }
                        else { HStack(spacing: 4) { Image(systemName: "chevron.left"); Text("戻る") } }
                    }
                }
            }
            .onAppear { viewModel.prepare(with: cards) }
            .onDisappear { speechService.stop(); convVM.stop() }
        }
    }
    ...
    private var modeSelectView: some View {
        VStack(spacing: 20) {
            Text("どのモードで練習しますか？").font(.title3.bold()).frame(maxWidth: .infinity, alignment: .leading)
            modeCard(title: "フラッシュカード",
                     description: "カードを見て自己評価\n覚えた・あやしい・まだ無理",
                     icon: "rectangle.on.rectangle", color: .blue) {
                selectedMode = .flashcard; withAnimation { step = .settings }
            }
            modeCard(title: "会話練習",
                     description: "相手のセリフに英語で返す\nAIが1問ずつ場面を生成",
                     icon: "bubble.left.and.bubble.right.fill", color: .green) {
                selectedMode = .conversation; withAnimation { step = .settings }
            }
            Spacer()
            Text("全 \(cards.count) 枚のカード").font(.caption).foregroundColor(.secondary)
        }
    }
    ...
    private func modeCard(title: String, description: String, icon: String, color: Color, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            HStack(spacing: 16) {
                Image(systemName: icon).font(.system(size: 28)).foregroundColor(color).frame(width: 48)
            ...
            .padding().background(Color.systemSecondaryBackgroundCompat)
    ...
    private var settingsView: some View {
        VStack(alignment: .leading, spacing: 24) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                Text("順番").font(.headline)
                HStack(spacing: 8) {
                    ForEach(LearnCardOrder.allCases) { ord in
                        let sel = viewModel.order == ord
    ...
    private var flashcardSessionView: some View {
        Group {
            if let card = viewModel.currentCard, !viewModel.sessionCompleted { learningContent(for: card) }
            else { completionView }
        }
    }

    // MARK: - ③ 会話練習本番（1問ずつAI生成）
    ...
    private var conversationSessionView: some View {
        ScrollView(showsIndicators: false) {
            VStack(spacing: 16) {
                switch convVM.phase {
                case .idle:              EmptyView()
                case .generating:        convGeneratingView
                case .error(let msg):    convErrorView(msg)
                case .finished:          convFinishedView
                default:
                    if let scene = convVM.currentScene {
                        VStack(spacing: 6) {
                            Text("\(convVM.progress)問目")
                                .font(.caption.weight(.semibold)).foregroundColor(.secondary)
                                .frame(maxWidth: .infinity, alignment: .trailing)
                            ProgressView(value: Double(convVM.currentIndex + 1),
                                         total: Double(max(convVM.cards.count, 1))).tint(.green)
                        }.padding(.horizontal)
                        convSceneView(scene: scene)
                        if case .nextReady = convVM.phase {
                            Button { convVM.goNext() } label: {
                                HStack(spacing: 8) {
                                    Text(convVM.isLastCard ? "完了" : "次の問題へ").fontWeight(.semibold)
                                    Image(systemName: convVM.isLastCard ? "checkmark.circle.fill" : "arrow.right.circle.fill")
                                }
    ...
    private func convSceneView(scene: ConversationScene) -> some View {
        VStack(spacing: 12) {
            convBubble(label: "相手", icon: "person.bubble.fill",
                       english: scene.partnerOpeningEnglish, japanese: scene.partnerOpeningJapanese,
                       onSpeak: { convVM.speakPartner() })
            convMyArea(scene: scene)
            if case .showReply = convVM.phase {
                convBubble(label: "相手（続き）", icon: "person.bubble.fill",
                           english: scene.partnerReplyEnglish, japanese: scene.partnerReplyJapanese,
                           onSpeak: { convVM.speakReply() })
            } else if case .nextReady = convVM.phase {
                convBubble(label: "相手（続き）", icon: "person.bubble.fill",
                           english: scene.partnerReplyEnglish, japanese: scene.partnerReplyJapanese,
                           onSpeak: { convVM.speakReply() })
            }
    ...
    private func convBubble(label: String, icon: String, english: String, japanese: String, onSpeak: @escaping () -> Void) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Label(label, systemImage: icon).font(.caption.weight(.semibold)).foregroundColor(.secondary)
    ...
    private func convMyArea(scene: ConversationScene) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            Label("あなた", systemImage: "person.fill").font(.caption.weight(.semibold)).foregroundColor(.secondary)
            switch convVM.phase {
            case .showOpening:
                VStack(alignment: .leading, spacing: 8) {
                    Text(scene.card.japanese).font(.body).foregroundColor(.secondary)
                    Text("↑ このフレーズを英語で返してみましょう").font(.caption).foregroundColor(.secondary)
                    Button { convVM.showHintAndCountdown() } label: {
                        Text("準備できた（シンキングタイムへ）").frame(maxWidth: .infinity).padding(.vertical, 4)
                    }.buttonStyle(.borderedProminent).tint(.green)
                }
            case .showHint, .countdown:
                VStack(alignment: .leading, spacing: 10) {
                    Text(scene.card.japanese).font(.title3.weight(.semibold)).fixedSize(horizontal: false, vertical: true)
                    HStack {
                    ...
                    Button { convVM.revealAnswer() } label: { Text("答えを見る").frame(maxWidth: .infinity).padding(.vertical, 4) }
                        .buttonStyle(.bordered)
                }
            case .showAnswer, .showReply, .nextReady:
                VStack(alignment: .leading, spacing: 8) {
    ...
    private var convGeneratingView: some View {
        VStack(spacing: 16) {
            ProgressView().scaleEffect(1.3)
            Text("AIが場面を生成中… (\(convVM.progress)問目)").font(.subheadline).foregroundColor(.secondary)
    ...
    private func convErrorView(_ msg: String) -> some View {
        VStack(spacing: 16) {
            ...
            Text(msg).font(.caption).foregroundColor(.secondary).multilineTextAlignment(.center).padding(.horizontal, 32)
            Button("もう一度試す") {
                Task { await convVM.start(
                    cards: viewModel.selectedCards.isEmpty ? cards : viewModel.selectedCards,
                    turnCount: viewModel.questionCount
    ...
    private var convFinishedView: some View {
        VStack(spacing: 24) {
            ...
            Text("\(convVM.cards.count)問の会話練習をしました").font(.subheadline).foregroundColor(.secondary)
            Button("もう一度") {
                Task { await convVM.start(
                    cards: viewModel.selectedCards.isEmpty ? cards : viewModel.selectedCards,
                    turnCount: viewModel.questionCount
            ...
            Button("設定に戻る") { withAnimation { step = .settings } }.buttonStyle(.bordered)
        }.padding()
    }
    ...
    private func learningContent(for card: WordCard) -> some View {
        VStack(spacing: 0) {
            progressSection; Spacer(); flashCard(for: card); Spacer()
            actionButtons(for: card)
            if viewModel.isShowingAnswer { evaluationButtons }
            navigationButtons
        }
    }
    ...
    private func flashCard(for card: WordCard) -> some View {
        ZStack {
            RoundedRectangle(cornerRadius: 20).fill(Color.systemSecondaryBackgroundCompat).shadow(radius: 5)
            VStack(spacing: 20) {
        ...
        }.frame(maxWidth: .infinity).frame(height: 320).padding(.horizontal, 20)
    }
    ...
    private func statusBadge(for card: WordCard) -> some View {
        HStack(spacing: 8) {
            Text(card.learningStatus.displayName)
                ...
                .foregroundColor(statusColor(for: card.learningStatus)).cornerRadius(999)
            if let next = card.nextReviewAt {
                Text("次回: \(next.formatted(date: .abbreviated, time: .omitted))").font(.caption).foregroundColor(.secondary)
            }
        }
    }
    ...
    private func actionButtons(for card: WordCard) -> some View {
        VStack(spacing: 12) {
            HStack(spacing: 12) {
                speechButton(title: "日本語", systemImage: "speaker.wave.2.fill") { speechService.speak(card.japanese) }
            ...
            Button {
                withAnimation(.spring(response: 0.3, dampingFraction: 0.7)) { viewModel.toggleAnswer() }
            } label: {
    ...
    private func speechButton(title: String, systemImage: String, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Label(title, systemImage: systemImage).frame(maxWidth: .infinity).padding()
                .background(Color.systemTertiaryBackgroundCompat).cornerRadius(12)
    ...
    private var evaluationButtons: some View {
        HStack(spacing: 12) {
            ForEach(LearnCardEvaluation.allCases, id: \.title) { evaluation in
                Button {
                    if let updated = viewModel.applyEvaluation(evaluation) { onUpdateCard?(updated) }
                } label: {
                    Text(evaluation.title).font(.subheadline.bold()).foregroundColor(.white)
    ...
    private var navigationButtons: some View {
        HStack(spacing: 20) {
            Button { viewModel.previousCard() } label: {
                HStack { Image(systemName: "chevron.left"); Text("前へ") }
                    .frame(maxWidth: .infinity).padding()
                    .background(Color.systemTertiaryBackgroundCompat)
                    .foregroundColor(viewModel.isFirstCard ? .gray : .primary).cornerRadius(10)
            }.disabled(viewModel.isFirstCard)
            Button {
                if viewModel.isLastCard { viewModel.sessionCompleted = true } else { viewModel.nextCard() }
            } label: {
                HStack {
                ...
                .foregroundColor(viewModel.isLastCard ? .white : .primary).cornerRadius(10)
            }
    ...
    private var completionView: some View {
        VStack(spacing: 20) {
            ...
            Button("もう一度") { viewModel.resetSession(with: cards); step = .settings }.buttonStyle(.borderedProminent)
            Button("閉じる") { dismiss() }
        }
    }
    ...
    private var emptyStateView: some View {
        VStack(spacing: 20) {
            Image(systemName: "square.stack.3d.up.slash").font(.system(size: 60)).foregroundColor(.secondary)
    ...
    private func statusColor(for status: LearningStatus) -> Color {
        switch status {
        case .new: return .blue; case .notSure: return .orange
        case .reviewing: return .purple; case .mastered: return .green
        }
    }
}
